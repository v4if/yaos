...
int printk(const char *fmt,...)
{
	 ...
	 * 对ASCII控制字符进行了忽略，于是对控制台打印函数又进行了修改
	 */
#endif
...
{
	char ch;
	char *err_msg = (char *)"The format of printk() is error\n";
	...
	while(*fmt)
	{
		...
		if(ch == '%')
		{
			...
			switch (ch){
				...
				//float
				case 'f' :
							float_template = (float)va_arg(args,double);
							byte_len += console_write_float(float_template);
							break;
				//format error
				default  :	//错误的参数类型
							console_puts_color(err_msg,vga_black,ALLERT_COLOR);
							return -1;
			}
		}
		else
